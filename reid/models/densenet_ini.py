...
class DenseNet(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16),
        ...
        self.hidden = nn.Sequential(OrderedDict([
            ...
            ('conv3', nn.Conv2d(num_features, num_features, kernel_size=3,
                                stride=1, padding=1, bias=False)),
            ('norm3', nn.BatchNorm2d(num_features)),
            ('relu3', nn.ReLU(inplace=True)),
            ('pool3', nn.MaxPool2d(kernel_size=2, stride=2, padding=1)),
            ('conv4', nn.Conv2d(num_features, num_features, kernel_size=3,
                                stride=1, padding=1, bias=False)),
        ...
        self.embedding = nn.Sequential(OrderedDict([
            ('feat', nn.Linear(3 * 6 * 512,hidden_num)),
    ...
    def forward(self, x):
        ...
        for name, module in self.features.items():
            if 'conv' in name and 'denseblock3' not in name and 'denseblock4' not in name:
                rec_feature = module(rec_feature)
            if 'pool' in name and 'denseblock3' not in name and 'denseblock4' not in name:
                rec_feature = module(rec_feature)
        hidden = self.hidden(rec_feature)
        hidden = hidden.view(hidden.size(0),-1)
        rec_feature = self.embedding(hidden)
        return x_class,rec_feature
